...
if len(sys.argv) != 2:
    print(f"Usage: python3 {sys.argv[0]} <dossier>")
    sys.exit(1)

photos_path = os.path.abspath(sys.argv[1])

if not os.path.isdir(photos_path):
    print(f"Erreur : le dossier n'existe pas : {photos_path}")
    sys.exit(1)


# Dériver base_path en supprimant tout jusqu'à 'static' inclus

base_index = photos_path.find('/static/')
...
def main():
    ...
    photos = [
        os.path.join(photos_path, f)
        for f in os.listdir(photos_path)
        if f.lower().endswith(('jpg', 'jpeg', 'png'))
    ]

    photos.sort(
        key=lambda x: read_photo_datetime(x) or datetime.min
    )
    ...
    for photo in photos:
        ...
        if (
            last_photo_datetime is None
            or photo_datetime - last_photo_datetime
            > timedelta(minutes=event_threshold)
        ):
            ...
            event_dir = os.path.join(
                photos_path,
                event_start.strftime('%Y-%m-%d_%H-%M-%S')
            )

            if not os.path.exists(event_dir):
                os.makedirs(event_dir)

                # Générer et afficher la ligne de sortie
