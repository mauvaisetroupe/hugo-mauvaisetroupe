...
var ttBgColor      = '#000033';
var ttBgImg        = '';           // path to background image;
var ttBorderColor  = '#003399';
var ttBorderWidth  = 1;
var ttDelay        = 400;          // time span until tooltip shows up [milliseconds]
var ttDelayAfter   = 200;
...
function tt_Htm(tt, t_id, txt, t_DelayAfter, t_stickyondiv)
{
	...
	var t_y = '<div onmouseover="divOnMouseOver(' + t_stickyondiv + ')" onmouseout="divOnMouseOut(' + t_DelayAfter + ',' + t_stickyondiv + ')" id="' + t_id + '" style="position:absolute;z-index:1010;';
		...
		var j = tags.length; while (j--)
		{
			if (typeof (t_tj = tags[j]).onmouseover == 'function' && t_tj.onmouseover.toString().indexOf(esc) != -1 && !tt_n6 || tt_n6 && (over = t_tj.getAttribute('onmouseover')) && over.indexOf(esc) != -1)
			{
				...
				htm += tt_Htm(
					...
					'tOoLtIp'+i+''+j,
					txt.wzReplace('& ','&'),
					((typeof t_tj.T_POSTDELAY != tt_u)? t_tj.T_POSTDELAY : ttDelayAfter),
					((typeof t_tj.T_STICKYONDIV != tt_u)? t_tj.T_STICKYONDIV : tt_stickyondiv)
				);
				...
				t_tj.onmouseout = new Function('e',
					'tt_Hide('+
					((typeof t_tj.T_POSTDELAY != tt_u)? t_tj.T_POSTDELAY : ttDelayAfter) +
					');'
				);
...
//lco
//arrete le timer
if (window.tt_rdl2) { window.clearTimeout(tt_rdl2);}
//appel de tt_hide2 direct
	if (tt_obj) tt_Hide2();
	...
	if (tt_obj)
	{
		...
		window.tt_rdl = window.setTimeout(
			...
			'{'+
				...
				'document.onmouseup = new Function("window.setTimeout(\'tt_Hide2();\', 0xa);");'+
			'}'+
...
function tt_Move(t_ev)
{
	...
	if (tt_op6)
	{
		if (tt_area && t_e.target.tagName != 'AREA') tt_Hide2();
		else if (t_e.target.tagName == 'AREA') tt_area = true;
	}
}


function tt_Hide(t_delayAfter)
{
	
	if (window.tt_obj)
	{
//lco		
		window.tt_rdl2 = window.setTimeout('tt_Hide2()', t_delayAfter);
//
	}
}
...
//lco
function divOnMouseOver(t_sticky_onmouseover) {
	if (t_sticky_onmouseover) {
		window.clearTimeout(tt_rdl2);
	}
}
function divOnMouseOut(t_delayAfter,t_sticky_onmouseover) {
	if (t_sticky_onmouseover) {
		window.tt_rdl2 = window.setTimeout('tt_Hide2()', t_delayAfter);
	}
}
